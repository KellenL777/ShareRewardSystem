...
export class ShareRewardService {
    ...
    private async filterAssetsByPrice(assets: Array<{ tickerSymbol:string }>, min: number, max: number) : Promise<TradableAsset[]> {
    ...
    private selectRangeBasedOnDistribution(distributionLevel: DistributionLevel): DistributionRange {
        ...
        let cumulative = 0;
        for (const key of Object.keys(distributionLevel)) {
            const distRange = distributionLevel[key];
